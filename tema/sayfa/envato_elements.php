<?php 
    $uyeid = $uyeRow["uye_id"];
    $paketlerims = $db->query("SELECT * FROM paketlerim  where hizmet = 2 and paket_durum = 1 and kisi_id = '$uyeid'");
    ...
    $fark = ($bitisTarihi - $baslangicTarihi) / 86400;
        ...
        $indirmeler = $db->query("SELECT * FROM indirmeler where hizmet = 2 and paket_id = '$paket_idd' and kisi_id = '$uyeid'");
        $total_downloads = $db->query("SELECT * FROM indirmeler where hizmet = 2 and kisi_id = '$uyeid'")->rowCount();
...
<div class="content-header row">
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-start mb-0">Envato Elements</h2>
                <div class="breadcrumb-wrapper">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="<?=URL?>"><?php echo $bloklar["site_link_map_home"]; ?></a>
                    </li>
                    <li class="breadcrumb-item active">Envato Elements
                    </li>
                </ol>
            </div>
        </div>
...
<section id="basic-input">
    <div class="row">
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Envato Elements</h4> <h4 class="card-title" style="float:right;">Servis Durumu : <?php if($bakim["envato_mod"]==0){echo '<button type="submit" class="btn btn-danger">Pasif</button>';}elseif($bakim["envato_mod"]==1){echo '<button type="submit" class="btn btn-success">Aktif</button>';} ?></h4>
                </div>
                <div class="card-body " >
                            ...
                            <div class="card mb-0">
                                <div class="card-body text-center">
                                    ...
                                    <p class="card-text mb-2">
                                        Envato Elements Hizmetine Sahip Değilsiniz!
                    ...
                    <?php }else{ ?>
                        <form role="form" class="form-horizontal" id="forms" method="POST" action="<?php echo TEMA_URL; ?>ajax.php?p=envato_elements"  enctype="multipart/form-data">
                        <div class="row">
                            <input type="hidden" name="uyeid" value="<?php echo $uyeRow["uye_id"]; ?>">
                            <input type="hidden" name="paket_id" value="<?php echo $paketlerimcek["id"]; ?>">
                            <input type="hidden" name="xjkhas" value="<?php echo $day_limit; ?>">
                            <input type="hidden" name="qwskji" value="<?php echo $paketlerceks["gil"]; ?>">
                            <div class="col-xl-12 col-md-12 col-12">
                                <div class="mb-1">
                                    <label class="form-label" >İçerik Url Giriniz</label>
                                    <input <?php if($day_limit == $paketlerceks["gil"]){echo"disabled";} ?> <?php if($bakim["envato_mod"]==0){echo "disabled";} ?> type="text" class="form-control" name="url" placeholder="İçerik Url Giriniz" />
                                </div>
                            </div>
                            <div class=" col-md-12">
                            <center>
                            <button type="submit" <?php if($bakim["envato_mod"]==0){echo "disabled";} ?>  <?php if($day_limit == $paketlerceks["gil"]){echo"disabled";} ?>  class="btn btn-success me-1 mt-2"><?php if($day_limit == $paketlerceks["gil"]){echo"Günlük İndirime Limitiniz Doldu!";}else{echo"İndirme Bağlantısı Oluştur";} ?></button>
                            </center>
                        </div>
                        </div>
                    </form>
...
$duyuruYesil = $db->query("SELECT * FROM duyurular WHERE hizmet = 2 ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC);
       ...
       success: function(data) {
            if (data == "bos-deger") {
                ...
                return false;
                 ...
                 Swal.fire({
                    title: 'Hata',
                    text: 'Lütfen Envato Linki Olduğundan Emin Olun !',
                    icon: 'info',
                    customClass: {
                      confirmButton: 'btn btn-info'
                    },
...
window.location.href = "<?php echo URL; ?>envato_elements/";
                  });
                return true;
...
IndirmeSifirla(<?php echo $uyeRow["uye_id"]; ?>);
    ...
    function IndirmeSifirla(catId) {
        $.post('<?php echo TEMA_URL; ?>ajax.php?p=IndirmeSifirla', {
            id: catId
        }, function(data) {});
    }
</script>
